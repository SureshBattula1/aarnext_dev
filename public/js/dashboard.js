(function($) {
  ...
  $(function() {
    ...
    $.ajax({
      url: public_path + '/dashboard/monthly-status',
      method: 'GET',
      success: function(response) {
        console.log("API Response:", response);
    
        const labels = response.labels.map(day => `Day ${day}`);
        const sales = response.sales.map(val => parseFloat(val));
        const collections = response.collections.map(val => parseFloat(val));
        ...
        const revenueChartCanvas = $("#order-chart").get(0).getContext("2d");

...
class PaymentsController extends Controller
{
            ...
            try {
                ...
                $query = DB::table('order_payments as op')
                    ...
                    ->select('op.id', 'op.user_id', 'op.payment_method', 'c.card_code' , 'c.card_name' , 'c.grp_name' ,
                     ...
                     'is_payment_approved', 'approved_by' , 'approved_at', 'remarks', 'approval_remarks')
    ...
    public function storeDirectPayments(Request $request) {
        ...
        try{
            ...
            $bankName = $request->input('bank_names');
            ...
            if(request('payment_method') !== 'cash'){
                ...
                }else if($cash > 0 && $paymentMethod === null){
                    $paymentMethod = 'cash';
                    $params['cash'] = $cash;
                }else if($online > 0 && $paymentMethod === 'tpa'){
                    $paymentMethod = 'tpa';
                    $params['online'] = $online;
                }else if($online > 0 && $paymentMethod === 'bank'){
                    $paymentMethod = 'bank';
                    $params['online'] = $online;
                    $params['is_payment_approved'] = 0;
                }
                ...
                $folderPath = 'attachments/payments';
                ...
                if ($request->file('attachments')) {
                    foreach ($request->file('attachments') as $file) {
                        if ($file->isValid()) {
                            ...
                            DB::table('order_payment_attachments')->insert([
            ...
            for ($i=0; $i < sizeof($request['select_invoice'])  ; $i++) {
                    ...
                    $balance_amount = $request['balance_amount'][$get_number];
    ...
    public function bankPaymentsApprovel($id, $action, Request $request){
        try {
            ...
            DB::table('order_payments')->where('id', $id)->update([
                'is_payment_approved'  => $status,
                'approved_by' => $userId,
                'approved_at' => now(),
    ...
    public function getAttachments($paymentId)
    {
        $attachments = DB::table('order_payment_attachments')
                        ->where('order_payment_id', $paymentId)
                        ->get()
                        ->map(function($att) {
                            $att->file_url = asset($att->file_path);
                            return $att;
                        });

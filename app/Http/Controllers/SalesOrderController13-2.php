        ...
        if (isset($data['draw'])) {
            ...
            try {
                ...
                ->leftjoin('sales_employees as s', 'q.sales_emp_id', '=', 's.id')
                    ->select('q.id','q.sales_ref_no', 'q.sales_invoice_no', 'q.customer_id', 'q.ref_id', 'q.ship_to', 'q.user_id', 'q.order_date', 'q.status', 'q.total', 'q.sales_invoice_no', 'q.bill_to','q.sales_type','c.card_name','c.grp_name','u.code','s.name','q.sales_emp_id',
                    DB::raw("(CASE
                        WHEN q.status = '0' THEN CONCAT('<span class=\"text-warning\"><b>', 'Pending', '</b></span>')
                        WHEN q.status = '1' THEN CONCAT('<span class=\"text-success\"><b>', 'Completed', '</b></span>')
                        ELSE CONCAT('<span class=\"text-muted\"><b>', 'Unknown', '</b></span>')

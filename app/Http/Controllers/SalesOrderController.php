    ...
    function SalesOrderJson(Request $request)
    {
        ...
        if (isset($data['draw'])) {
            ...
            try {
                ...
                ->leftJoin('users as ua', 'q.approved_by', '=', 'ua.id')
                ->leftjoin('sales_employees as s', 'q.sales_emp_id', '=', 's.id')
                    ...
                    'q.sales_type','c.card_name','c.grp_name','u.code','s.name','q.sales_emp_id', 'q.is_sales_approved', 'q.approval_remarks', 'ua.name as approved_by', 'q.approved_at','approval_type', 
                    DB::raw("(CASE
   ...
   // echo "GOPI";
    ...
    public function storeOrderQuotation(Request $request)
    {
        try {
            ...
            // echo "<pre>"; print_r($custGrpCode); exit;
            // $request = $request->all();
            // exit;

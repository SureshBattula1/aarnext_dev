    ...
    function generateInvoicePDF($invoiceNo)
{
    ...
    // Fetch invoice with customer and sales employee info
    $invoice = DB::table('invoices')
        ->join('customers as c', 'invoices.customer_id', '=', 'c.card_code')
    ...
    // Fetch invoice items and batches
    $items = DB::table('invoice_items')->where('invoice_id', $invoice->id)->get();
    $invoiceBatches = DB::table('invoice_batches')->where('invoice_id', $invoice->id)->get();
    ...
    // Calculate totals and attach batches to items
    foreach ($items as $item) {
        $price = $item->price;
        $quantity = $item->quantity;
        $disc_percent = $item->disc_percent;
    ...
    // Generate PDF
    $pdf = PDF::loadView('pdf.new-format-pdf', compact(
    ...
    $ownerPassword = 'owner123'; // must be non-empty
    $canvas->get_cpdf()->setEncryption($ownerPassword, null, $permissions);


    $canvas->page_text(500, 800, "Page {PAGE_NUM} of {PAGE_COUNT}", 'helvetica', 10, [0,0,0]);

    // Stream PDF inline to browser (prevents automatic download)
    return $pdf->stream('invoice_' . $invoice->id . '.pdf', ['Attachment' => false]);
}
    ...
    {
        $privateKeyFile = public_path('haskey/b1-saft-private.pem');

...
class ItemQuotationController extends Controller
{
    ...
    function getItemQuotationsJson(Request $request)
    {
        ...
        if (isset($data['draw'])) {
            $columnArray = ['q.id', 'q.date', 'q.customer_code', 'q.total', 'q.quotation_pdf_no','q.user_id','q.quotation_ref_no','c.card_name','q.sales_type','q.total','q.status' ,'c.grp_name','u.code','q.sales_emp_id'];
            ...
            try {
                ...
                ->select('q.id', 'q.date', 'q.customer_code', 'q.total', 'q.quotation_pdf_no','q.user_id','q.quotation_ref_no','q.sales_type','c.card_name','q.total','q.status' , 'c.grp_name','u.code','s.name','q.sales_emp_id',
                        DB::raw("(CASE
                ...
                END) as customer_code_display"),
                    DB::raw("(SELECT SUM(price_total_af_disc) as total_price FROM `order_quotation_items` WHERE `quotation_id` = q.id) as total_price"),
                    DB::raw("(SELECT SUM(gst_value * quantity) as total_tax FROM `order_quotation_items` WHERE `quotation_id` = q.id) as total_tax"),
                    DB::raw("(SELECT SUM(subtotal) FROM `order_quotation_items`  WHERE `quotation_id` = q.id) as total_with_tax_sum")
                    )
    ...
    public function storeQuotation(Request $request)
    {
        try {
            ...
            DB::commit();
            $QuotationpdfUrl = $this->generateQuotationPDF($orderId);
    ...
    // function generateQuotationPDF($QuotationNumber)
    // {
    //     $warehouseCode = Auth::user()->ware_house;
    ...
    //      // Initialize GST summary and total
    //      $total = 0;
    //      $gstSummary = [];
    ...
    function generateQuotationPDF($QuotationNumber)
    {
        ...
        $quotationItems = DB::table('order_quotation_items')->where('quotation_id', $quotation->id)->get();
         ...
         $totalPages = ceil(count($quotationItems) / $itemsPerPage);

        $pdf = PDF::loadView('pdf.items-quotation-pdf', compact('quotation', 'quotationItems','warehouseCode' , 'gstSummary','totalPages','custAddress','ware_house_data'))
                    ->setPaper('a4')

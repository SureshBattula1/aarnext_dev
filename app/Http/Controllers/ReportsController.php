...
class ReportsController extends Controller
{
            ...
            if ($request->sales_report === "invoices") {
                ...
                $query = DB::table('invoices as q')
                    ...
                    ->select([
                        ...
                        'q.sap_updated_at', 'q.sap_updated', 'c.grp_name', 'u.code','u.name as user_name',  'c.sub_grp','s.code', 's.name as sales_employee_name', 's.memo'
                    ])
                    ->orderBy('q.id', 'DESC');
    ...
    //        if (!empty($data['doc_num_search'])) {
    //            $searchTerm = '%' . $data['doc_num_search'] . '%';
    //            $query->where('rft.rft_doc_num', 'LIKE', $searchTerm);
    //        }
    ...
    public function paymentsJson(Request $request){
        ...
        try {
            ...
            foreach ($rawOrders as $row) {
                if (!isset($orders[$row->id])) {
                    $orders[$row->id] = [
                        ...
                        'bank_tpa' => $row->bank_tpa,
                        'card_name' => $row->card_name,
                        'sub_grp' => $row->sub_grp,
                        'user_name' => $row->user_name,
                        'ware_house' => $row->ware_house,
                        'invoices' => []
                    ];
                }
    
                if ($row->invoice_id) {
                    $orders[$row->id]['invoices'][] = [
                        'id' => $row->invoice_id,
            ...
            // ✅ Now calculate totals AFTER $orders is built
            foreach ($orders as $order) {
                $count = count($order['invoices']);
                if ($count > $maxInvoiceCount) {
                    $maxInvoiceCount = $count;
                }
            }            
            ...
            return response()->json([
                ...
                'avgInvoiceCount' => $maxInvoiceCount
            ]);
            ...
            if ($salesReportType === "invoices") {
                ...
                $salesDataQuery = DB::table('credit_memos as cm')
                    ...
                    ->select([
                        ...
                        'cm.customer_id',
                        'c.card_name',
                        DB::raw('LOWER(i.sales_type) as sales_type'),  
                        'cm.order_date', 
                        'cm.total',
                        'cm.paid_amount',

...
use Exception;
class SapInvoicePostByAltBenController extends Controller
{
    ...
    {
       $user_session = DB::table('users')->select('session_key')->where('id',81)->first();
          if ($user_session) {
    ...
    {
   $user_session = DB::table('users')->select('session_key')->where('id',82)->first();
      if ($user_session) {
            $session_id = $user_session->session_key;
        try {
            $invoice = DB::table('invoices')
    ...
    {
   $user_session = DB::table('users')->select('session_key')->where('id',83)->first();
      if ($user_session) {
            $session_id = $user_session->session_key;
        try {
            $invoice = DB::table('invoices')
    ...
    {
   $user_session = DB::table('users')->select('session_key')->where('id',84)->first();
      if ($user_session) {
            $session_id = $user_session->session_key;
        try {
            $invoice = DB::table('invoices')

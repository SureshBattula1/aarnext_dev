...
class SapInvoicePostByAarUigController extends Controller
{
    ...
    {
      $user_session = DB::table('users')->select('session_key')->where('id',69)->first();
          if ($user_session) {
    ...
    {
      $user_session = DB::table('users')->select('session_key')->where('id',70)->first();
          if ($user_session) {
                $session_id = $user_session->session_key;
            try {
            $invoice = DB::table('invoices')
    ...
    {
      $user_session = DB::table('users')->select('session_key')->where('id',71)->first();
          if ($user_session) {
                $session_id = $user_session->session_key;
            try {
            $invoice = DB::table('invoices')

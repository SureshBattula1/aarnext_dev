...
class SapInvoicePostByAarBieController extends Controller
{
    ...
    {
        $user_session = DB::table('users')->select('session_key')->where('id',13)->first();
          if ($user_session) {
    ...
    {
    $user_session = DB::table('users')->select('session_key')->where('id',14)->first();
      if ($user_session) {
            $session_id = $user_session->session_key;
        try {
            $invoice = DB::table('invoices')
    ...
    {
    $user_session = DB::table('users')->select('session_key')->where('id',15)->first();
      if ($user_session) {
            $session_id = $user_session->session_key;
        try {
            $invoice = DB::table('invoices')

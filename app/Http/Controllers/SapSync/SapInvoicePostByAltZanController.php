...
use Exception;
class SapInvoicePostByAltZanController extends Controller
{
    ...
    {
        $user_session = DB::table('users')->select('session_key')->where('id',92)->first();
          if ($user_session) {
    ...
    {
    $user_session = DB::table('users')->select('session_key')->where('id',93)->first();
      if ($user_session) {
            $session_id = $user_session->session_key;
        try {
            $invoice = DB::table('invoices')
    ...
    {
        $user_session = DB::table('users')->select('session_key')->where('id',94)->first();
          if ($user_session) {
                $session_id = $user_session->session_key;
            try {
            $invoice = DB::table('invoices')

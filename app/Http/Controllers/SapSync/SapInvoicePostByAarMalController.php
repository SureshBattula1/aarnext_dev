...
class SapInvoicePostByAarMalController extends Controller
{
    ...
    {
    $user_session = DB::table('users')->select('session_key')->where('id',38)->first();
      if ($user_session) {
    ...
    {
    $user_session = DB::table('users')->select('session_key')->where('id',39)->first();
      if ($user_session) {
            $session_id = $user_session->session_key;
        try {
            $invoice = DB::table('invoices')
    ...
    {
    $user_session = DB::table('users')->select('session_key')->where('id',40)->first();
      if ($user_session) {
            $session_id = $user_session->session_key;
        try {
            $invoice = DB::table('invoices')

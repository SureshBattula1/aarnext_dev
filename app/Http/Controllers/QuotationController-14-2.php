...
class QuotationController extends Controller
{
    ...
    function getQuotationsJson(Request $request)
    {
        ...
        if (isset($data['draw'])) {
            $columnArray = [
                ...
                'q.invoice_no', 'q.ref_id', 'q.user_id', 'q.bill_to', 'q.ship_to', 'c.card_name','q.store_code','q.due_date','q.pymnt_grp', 'q.sap_created_id' , 'q.sap_error_msg' , 'q.sap_updated_at', 'q.sap_updated','c.grp_name','u.code','s.name','q.sales_emp_id',
            ];
            ...
            try {
                ...
                DB::raw("(CASE
                WHEN q.sap_updated = '0' THEN CONCAT('<span class=\"text-warning\"><b>', 'Pending', '</b></span>')
                WHEN q.sap_updated = '1' THEN CONCAT('<span class=\"text-success\"><b>', 'Updated', '</b></span>')
                ELSE CONCAT('<span class=\"text-muted\"><b>', 'Unknown', '</b></span>')
            END) as sap_status"),
                        DB::raw("(SELECT SUM(price_total_af_disc) as total_price FROM `invoice_items` WHERE `invoice_id` = q.id) as total_price"),
                        DB::raw("(SELECT SUM(gst_value * quantity) as total_tax FROM `invoice_items` WHERE `invoice_id` = q.id) as total_tax"),
                        DB::raw("(SELECT SUM(subtotal) FROM `invoice_items`  WHERE `invoice_id` = q.id) as total_with_tax_sum"),
    ...
    public function getItemSearch($type = null, Request $request)
    {
        ...
        //$tot_qty =  $batches->select(DB::raw("SUM(quantity) as total_qty"));
        ...
        $query = DB::table('items as i')
                    ...
                    ->select(
                        ...
                        DB::raw("
                            CONCAT(
                                ...
                                ' - ',
                                '<span style=\"color:gray; font-weight:bold; font-size:11px;\">', ROUND(COALESCE(SUM(ws.quantity) - SUM(ws.invoice_qty) + SUM(ws.credit_qty), 0), 0), '</span>'
                            ) as text
                        "),
                        'i.item_name',
                        'i.item_code',
                    ...
                    ->groupBy('i.item_code', 'ip.price', 'i.id') // Group by necessary fields to avoid duplicate results
                    ->distinct()
                    ->orderBy('i.item_code')
                    ->get();
    ...
    function getItem($itemId)
    {
        ...
        try {
            ...
            ->select(
                ...
                //DB::raw("
                //    CASE
                //        WHEN REGEXP_LIKE(i.others_iva, '[0-9]') THEN REGEXP_REPLACE(i.others_iva, '[^0-9]', '')ELSE 0
                //        END AS tax
                //"),
                //DB::raw("IF(STRCMP(i.others_iva, '') != 0 AND i.others_iva REGEXP '^[0-9]+$', i.others_iva, 0) as tax"),
...
public function invoiceBatchList($invoiceId , $itemCode){
    ...
    public function HashkeyGenerationInvoice($inv_details)
        {
            $warehouseCode = Auth::user()->ware_house;

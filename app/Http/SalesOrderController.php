...
class SalesOrderController extends Controller
{
    ...
    public function orderApprovalIndex($orderId)
    {
        $orderData = DB::table('orders')
            ->select('id', 'customer_id', 'sales_invoice_no', 'total', 'is_sales_approved as status')
            ->where('id', $orderId)
            ->first();
        ...
        return view('salesorder.order_approvals', compact('orderData'));
    }
        ...
        if (isset($data['draw'])) {
            ...
            try {
                ...
                ->leftjoin('sales_employees as s', 'q.sales_emp_id', '=', 's.id')
                    ...
                    'q.sales_type','c.card_name','c.grp_name','u.code','s.name','q.sales_emp_id', 'q.is_sales_approved', 'q.approval_remarks', 'q.approved_by', 'q.approved_at','q.approval_type',
                    DB::raw("(CASE
                ...
                if (!empty($data['approval_search'])) {
                    if ($data['approval_search'] == "0") {
                        $query->where('q.is_sales_approved');
                    } elseif ($data['approval_search'] == "1") {
                        $query->where('q.is_sales_approved');
                    } elseif( $data['approval_search'] == "2") {
                        $query->where('q.is_sales_approved');
    ...
    public function searchSalesCustomer(Request $request)
{
   ...
   // echo "GOPI";
    ...
    public function getSalesOrderData($orderId) {
        ...
        $order = DB::table('order_items as oi')
            ...
            $response = [
                'sales_employee_name' => $remarks->sales_employee_name,
                'remarks'=> $remarks->remarks,
                'order_tot_amount' => $remarks->total,
                'sales_type' => $remarks->sales_type,
    ...
    //         $custGrpCode = $request->customer_grp_code;
    //         $cardCode = $request->customer_id;
    ...
    //         if($roleId === 5 || $request->sales_type === 'credit'){
    //             $orderUpdate = [
    //                 'is_sales_approved' => 0,
    //             ];
    //         }
    ...
    //                 'store_type' => $warehouseCode,
    //                 'doc_num' => $invoiceNo,
    //                 'order_id'=> $orderId,
    //             ];
    //             $emailController = new StockEmailController();
    //             $emailController->CreditCustomer($whatsappData);
    //         }
    //         DB::commit();
    //         $response = [
    //             'success' => true,
    ...
    public function storeOrderQuotation(Request $request)
    {
        try {
            ...
            foreach ($request->row_item_no as $index => $itemId) {
                ...
                if (!$itemInfo) throw new \Exception('Item not found for ID: ' . $itemId);
            ...
            if($roleId === 5 || $request->sales_type === 'credit'){
                $orderUpdate['is_sales_approved'] = 0; 
            }
            ...
            if($custGrpCode == 103 || $custGrpCode == 109){
                $orderUpdate['is_sales_approved'] = 0;  
                $orderUpdate['approval_type'] = 'credit';

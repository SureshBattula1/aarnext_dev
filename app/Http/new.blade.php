...
<form id="new_quotation_form" action="javascript:void(0)" method="POST" autocomplete="off">
    ...
    <input type="hidden" id="salesOrderTotValue" name="salesOrderTotValue">
    ...
    $(document).ready(function () {
        $("#new_quotation_form").validate({
            ...
            submitHandler: function (form) {
                ...
                // Check if grand_total exceeds salesOrderTotValue
                let grandTotal = parseFloat($('#grand_total').text()) || 0;
                let salesOrderTotValue = parseFloat($('#salesOrderTotValue').val()) || 0;
                if (grandTotal > salesOrderTotValue) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Invoice Total value more than sales order value',
                        showConfirmButton: true,
                    });
                    return false;
                }
            ...
            element.select2({
                    ajax: {
                        ...
                        , data: function(params) {
                            ...
                            var custGrpCode = $('#customer_grp_code').val();
                            console.log('custGrpCode', custGrpCode);

                            if (custGrpCode == 103 || custGrpCode == 109) {
                                // Show specific validation for credit customers
                                $('.customerSec').addClass('validation-error');
                                $('#customerError')
                                    .text('Credit customers cannot generate invoice directly.')
                                    .show();
                            ...
                            // ✅ For all other customers — normal validation
                            if (!cardCode) {
                                $('.customerSec').addClass('validation-error');
                                $('#customerError')
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response) {
                ...
                let SalesOrderTotal = response.order_tot_amount;
                ...
                $('#salesOrderTotValue').val(SalesOrderTotal);
                $('#remarks').val(remarks);
                $('#ref_no').val(RefNO);

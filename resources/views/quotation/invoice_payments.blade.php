    ...
    <div class="container">
        ...
        <div class="generateBtn">
            <div class="row">
                ...
                <div class="col-md-3">
                    @if($invoice->delivary_note_no)
                        <button type="button" class="btn btn-primary" onclick="DelivaryNote({{$invoice->id}})">Delivary Note</button>
                    @else 
                        <button type="button" id="generateDelivaryBtn" class="btn btn-primary" onclick="GenerateDelivaryNote({{$invoice->id}})">Generate Delivary Note</button>
                    @endif
                </div>
                ...
                <div class="col-md-3">
                    ...
                    @if(isset($invoice->generated_pdf))
                        <button type="button" class="btn btn-primary" onclick="GeneratePdf({{$invoice->id}})">Generate Invoice</button>
                    @else 
                        <button type="button" id="generateDelivaryBtn" class="btn btn-primary" onclick="GeneratePdf({{$invoice->id}})">Generate Invoice</button>
                    @endif
                                ...
                                @if (isset($invoiceItems) && $invoiceItems->count())
                                    @foreach ($invoiceItems as $item)
                                        <tr>
                                            ...
                                            <td style="word-wrap: break-word; white-space: normal;">{{ $item->item_desc }}</td>
                                            <td>{{ $item->quantity }}</td>
                                            <td>{{ $item->price }}</td>
                                ...
                                <td>
                                    ...
                                    <div class="invoice-summary">
                                        <div class="invoice-column">
                                            ...
                                            <span class="value">{{ number_format($invoice->total, 2) ?? '0' }}</span>
                                        </div>
                                        <div class="invoice-column">
                                            <strong class="label">PAID:</strong>
                                            <span class="value">{{ number_format($invoice->paid_amount, 2) ?? '0' }}</span>
                                        </div>
                                        @if ($invoice->paid_amount > 0)
                                            <div class="invoice-column">
                                                <strong class="label">BALANCE:</strong>
                                                <span
                                                    class="value">{{ number_format($invoice->total - $invoice->paid_amount, 2) }}</span>
                                            </div>
                                        @endif
                                    </div>
                                </td>
...
<script>
    function GenerateDelivaryNote(invoiceId) {
        var button = document.getElementById("generateDelivaryBtn");
        button.disabled = true; 
        button.innerHTML = "Processing..."; // Change text to Processing

        var url = "{{ url('generate_delivary_pdf') }}/" + invoiceId;
        ...
        .then(data => {
            if (data.success) {
                ...
                button.innerHTML = "Delivery Note Generated"; 
                
            } else {
                alert(data.message);
                button.innerHTML = "Generate Delivery Note"; 
    ...
    function DelivaryNote(invoiceId) {
        var url = "{{ url('pdf/delivaries') }}/delivary_copy_" + invoiceId + ".pdf";
